...
class LogisticRegression:
    ...
    def fit(self, 
        ...
        self.intercept = float(self.weights[-1])
...
class FLD:
    ...
    def fit(
        ...
        b = -0.5 * (self.m0 @ np.linalg.inv(self.sw) @ self.m0.T) + 0.5 * (self.m1 @ np.linalg.inv(self.sw) @ self.m1.T)
        print("b:", b)
        self.intercept = -b/self.w[1]
        print("intercept:", -b/self.w[1])
    ...
    def plot_projection(self, inputs: npt.NDArray[np.float_]):
        ...
        y0 = np.dot(x0, self.w)
        y1 = np.dot(x1, self.w)
        print("x00:", x0[:, 0].reshape(-1,1).shape)
        print("add:", np.add(x0[:, 0].reshape(-1,1),y0/self.slope).shape)
        plt.scatter(self.slope, y0, color='red')
        plt.scatter(y1/self.slope, y1, color='blue')
        # for i in range(inputs.shape[0]):
        #     plt.plot()

        x = [np.min(inputs[:, 0]), np.max(inputs[:, 0])]
        y = [np.min(inputs[:, 1]), np.max(inputs[:, 1])]
